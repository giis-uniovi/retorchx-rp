...
2. `/scripts`: contains all the necessary Python scripting code for average datasets are used as input for the cost
   model sheets.

3. `/tests`: contains different unit test cases that checks the different methods contained in the `/scripts`
   directory processing and calculating the average datasets from the raw files.

4. `/generatedatasets.sh/ps1`:  scripts used to launch the data processing and calculation tasks within the
   project.
...
6. `/profile-template.xlsx`: spreadsheet template that enables the Usage Profile calculations
...
of several test resources, such as web servers, databases, and multimedia servers that allows to create online classrooms,
classes or publish and create class resources.
...
test,
cases in these repositories. The test suite, along with the necessary scripts and JenkinsFile required to run the
Execution Plan, is made
available in a [GitHub repository](https://github.com/giis-uniovi/retorch-st-fullteaching).
...
2. Create a new Usage Profile stylesheet using the [template provided](profile-template.xlsx) and rename it with an
   appropriate name (e.g. OriginalexecutionplanProfile.xlns) and fine-tune the different parameters(outlined in red) :
    ...
    - COI Capacities and Invoiced Prices:
        ...
        - Billing_Option.Billing_Period: The time period for which billing is calculated.
...
3. Make as many copies of the Spreadsheet sheet as Cloud Object Instances, adjust the parameters accordingly, in order
   get its different costs and usage profiles and compare and analyze them.
...
- The Usage Profiles as well as the different utilization and costs (overall,set-up, test execution, tear-down and
  over provisioning) are shown in the button of the different sheets :
